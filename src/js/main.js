...
const sendForm = () => {
  ...
  const socialClass = document.querySelector('#socialClass').value
  const hasChronicDisease = document.querySelector(
    '[name="hasChronicDisease"]'
  ).value
  const isSocialSecurityUser = document.getElementById(
    '[name="isSocialSecurityUser"]'
  ).value

  const predictionData = {
    age: age,
    social_class: socialClass,
    public_user: isSocialSecurityUser === 'on' ? 1 : 0,
    long_term_illnes: hasChronicDisease === 'on' ? 1 : 0,
  }

  const url = 'localhost:5000/satisfaction'
